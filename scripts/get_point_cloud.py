...
def get_depth_cloud(disparity, baseline, f):
    ...
    if video_capture_left.isOpened() and video_capture_right.isOpened():
        try:
            ...
            while True:
                ...
                if filter_img == True:
                    frame_left = cv.filter2D(src=frame_left, ddepth=-1, kernel=kernel_blur)
                    frame_right = cv.filter2D(src=frame_right, ddepth=-1, kernel=kernel_blur)
                ...
                X = (c_mesh - cols/2)*(depth/f)
                Y = (r_mesh - rows/2)*(depth/f)
                Z = depth
